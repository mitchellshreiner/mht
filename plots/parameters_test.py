...
def run_test():
    """Run through training set with different parameters for clusters"""
    ...
    for i in range(1, 200):
        ...
        while(detFile['frame'][det_indexes[index]] == i):
            target.append(np.array([detFile['bb_left'][det_indexes[index]], detFile['bb_top'][det_indexes[index]]]))
            index = index + 1
    ...
    tracker = mht.MHT(
        cparams=mht.ClusterParameters(k_max=200, hp_limit=10),
    ...
    for report in frames:
        ...
        reports = {mht.Report(
            #np.random.multivariate_normal(t, np.diag([1.0, 1.0])),  # noqa
            # t[0:2],
            t,
            np.eye(2) * 0.001,
        ...
        if k <= 10:
            #print(list(list(tracker.global_hypotheses())[0].tracks)[1].filter)
            #print(h)
            #print(o)
            #print(C)
    ...
    file = open("parameters_test.out.txt", "w")
